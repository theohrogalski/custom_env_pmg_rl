...
class eval_type():
        ...
        while env.agents and num_iters<max_iters:
            ...
            for agent in env.agents:
                if action_freeze[agent]==0:
                    if env.graph.nodes[env.agent_position[agent]]["target"]==1:
    ...
    def full_model(self):
        env = GraphEnv()
        "Evaluation for a pre-determined checkpoint for the full algorithm. Uses the self.ckpt"
        ...
        while env.agents:
            ...
            for agent in env.agents:
                logits, value, x_state, edges = obs_net[agent](env.mental_map[agent], env.action_mask_to_node[int(agent[6:])],env.agent_to_net[agent], num_moves=env.num_moves)
            ...
            _,_,_,_,_,_ = env.step(actions)
        print(f"Total uncertainty for {self.ckpt} is total_uncertainty_everm")
    ...
    def random(self):
        ...
        while env.agents and num_iters<max_iters:
            if env.num_moves%200==0 and env.num_moves!=0:
                ...
                uncertainty_history=[]
                num_iters+=1
                print(f"Num iters for Random at {num_iters+1}/100")
            for agent in env.agents:
                actions[agent]=torch.tensor(random.sample(self.get_legal_actions(env.agent_position[agent],env.action_mask_to_node),1))
            _, _, _,_, _ = env.step(actions)
            uncertainty_history.append(env.tot_unc)
...
if __name__=="__main__":
    eto = eval_type("checkpoint_ep_200_training_session_one_1591")
